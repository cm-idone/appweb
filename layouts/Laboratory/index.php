        ...
        <tbody>
            <?php foreach ($global['custody_chains'] as $value) : ?>
                <tr>
                    ...
                    <td class="mediumtag"><?php echo ((($value['type'] == 'covid_pcr' OR $value['type'] == 'covid_an' OR $value['type'] == 'covid_ac') AND empty($value['employee'])) ? (($value['closed'] == true) ? '<i class="fas fa-envelope" style="margin-right:5px;color:#009688;"></i> {$lang.sended} | <a href="{$path.uploads}' . $value['pdf'] . '" download="' . $value['pdf'] . '">Descargar PDF</a>' : '<i class="fas fa-envelope"  style="margin-right:5px;color:#ff9800;"></i> {$lang.not_sended}') : '({$lang.internal})'); ?></td>
